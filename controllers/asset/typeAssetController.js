const typeAssetModel = require('../../models/typeAssetModel');
const { validationResult } = require('express-validator');
const express = require('express');
const flash = require('express-flash');
const session = require('express-session');

const typeAssetController = {};

typeAssetController.index = function (req, res, next) {
  typeAssetModel.getAllType(function (err, types) {
    if (err) {
      return next(err);
    }
    res.render('asset_type/index',{ title: 'Type Listing', types: types });
  });
};
...
module.exports = typeAssetController;
