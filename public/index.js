...
viewer.addEventListener(Autodesk.Viewing.GEOMETRY_LOADED_EVENT, async () => {
    const doc = viewer.model.getDocumentNode();
    const bubbleNode = doc.getRootNode();

    // Now you can use Document.getAecModelData(bubbleNode)
    const aecModelData = doc.getAecModelData(bubbleNode);
    console.log(aecModelData);
    // Initialize the timeline
    initTimeline(document.getElementById('timeline'), onTimeRangeChanged, onTimeMarkerChanged);
    // Activate XLSExtension
    const xlsExtension = viewer.getExtension("XLSE.Extension");
    if (xlsExtension) {
        xlsExtension.activate();
    }
    ...
    const extensions = [SensorListExtensionID, SensorSpritesExtensionID, SensorDetailExtensionID, SensorHeatmapsExtensionID, XLSExtensionID].map(id => viewer.getExtension(id));
